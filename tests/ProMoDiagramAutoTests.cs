...
namespace WinProMo_App.Tests
{
    ...
    public class ProMoDiagramAutoTests : AutomationTestDiagrams
    {
        ...
        {
            var types = diagram1.CreatableElementTypes;
            Assert.IsNotNull(types);
            Assert.IsTrue(types.Length > 0, "Expected at least one creatable element type");
            Assert.IsTrue(((object[])types).Cast<string>().Contains("promo_block_view"), "Expected creatable element type 'promo_block_view' not found");
            Assert.IsTrue(((object[])types).Cast<string>().Contains("promo_edge_view"), "Expected creatable element type 'promo_edge_view' not found");
